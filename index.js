  ...
  require('dotenv').config(); // local only; Render uses injected env vars
  ...
  // ✅ Cloudinary (must be defined before SSR + autopick uses it)
...
console.log('[env] MONGO_URI present?', !!MONGO_URI);

async function connectMongo() {
  try {
    await mongoose.connect(MONGO_URI, { dbName: 'newsdb', autoIndex: true });
...
if (picked?.chosen) {
  finalImagePublicId = picked.chosen.imagePublicId;
  finalImageUrl      = picked.chosen.imageUrl;
  finalOgImage       = picked.chosen.ogImage;
}
...
async function ensureArticleHasImage(payload = {}) {
  ...
  app.get('/api/articles/search', optionalAuth, withValidation(ArticlesListQuerySchema, 'query'), async (req, res) => {
   ...
   // ✅ Category filter should use categorySlug (same as /api/articles)
if (catRaw && catRaw.toLowerCase() !== 'all') {
  match.categorySlug = String(catRaw).trim().toLowerCase();
}
...
if (picked?.chosen) {
  ...
  // ✅ Fix category update (no undefined vars)
if (category !== undefined || categorySlug !== undefined || categoryName !== undefined) {
  const categoryInput = String(category ?? categoryName ?? categorySlug ?? 'General').trim() || 'General';

  const nameProbe = categoryInput;
  ...
  const catDoc = await Category.findOne({ $or: ors }).select('name slug').lean();

  update.category = catDoc ? catDoc.name : nameProbe;
  update.categorySlug = catDoc
    ? String(catDoc.slug || '').toLowerCase()
    : (slugify(update.category, { lower: true, strict: true }) || 'general');
}
  ...
  const uploadOptions = {
    ...
    resource_type: 'image',
    overwrite: false,
    tags: Array.from(new Set(autoTags)).slice(0, 20)
  };
...
async function startServer() {
  // ✅ connect DB first (prevents SSR/API random failures)
  await connectMongo();
  ...
  app.listen(PORT, HOST, () => {
    ...
    try {
      // do NOT print secret values; only print status
      console.log("[debug] AI_NEWS_CRON_ENABLED =", String(process.env.AI_NEWS_CRON_ENABLED || "false"));
